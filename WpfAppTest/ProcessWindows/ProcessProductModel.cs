...
namespace EditorInterface.ProcessWindows
{
    public class ProcessProductModel : INotifyPropertyChanged
    {
        public ProcessProductModel(ProcessProduct product)
        {
            ...
            Optional = product.Tags.Any(x => x.Tag == ProductionTag.Optional);
            if (Optional)
                OptionalBonus = (decimal)product.Tags.Single(x => x.Tag == ProductionTag.Optional)[0];
            Consumed = product.Tags.Any(x => x.Tag == ProductionTag.Consumed);
            Fixed = product.Tags.Any(x => x.Tag == ProductionTag.Fixed);
            Investment = product.Tags.Any(x => x.Tag == ProductionTag.Investment);
            Pollutant = product.Tags.Any(x => x.Tag == ProductionTag.Pollutant);
            Chance = product.Tags.Any(x => x.Tag == ProductionTag.Chance);
            if (Chance)
            {
                ChanceGroup = (char)product.Tags.Single(x => x.Tag == ProductionTag.Chance)[0];
                ChanceWeight = (int)product.Tags.Single(x => x.Tag == ProductionTag.Chance)[1];
            }
            Offset = product.Tags.Any(x => x.Tag == ProductionTag.Offset);
            DivisionCapital = product.Tags.Any(x => x.Tag == ProductionTag.DivisionCapital);
            DivisionInput = product.Tags.Any(x => x.Tag == ProductionTag.DivisionInput);
            AutomationCapital = product.Tags.Any(x => x.Tag == ProductionTag.AutomationCapital);
            AutomationInput = product.Tags.Any(x => x.Tag == ProductionTag.AutomationInput);
        }
